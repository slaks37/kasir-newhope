import React, { useState } from 'react';
import {
  ShieldCheck,
  Users,
  Store,
  KeyRound,
  Shield,
  CheckCircle2,
  Lock,
  UserCheck,
  Briefcase,
  UserCog,
  Building2,
  Filter,
} from 'lucide-react';
import { Card } from '../ui';
import { SECTOR_LABEL, Sector } from '../api';

export interface AdminUser {
  id: string;
  email: string;
  fullName: string;
  role: 'ROLE_SUPERADMIN' | 'ROLE_INTERNAL_GROWTH' | 'ROLE_INTERNAL_SUPPORT';
  scope: string;
  status: 'ACTIVE' | 'SUSPENDED';
  createdAt: string;
}

export interface ClientUser {
  id: string;
  storeName: string;
  businessSector: Sector;
  name: string;
  username: string;
  role: 'ADMIN' | 'MANAGER' | 'CASHIER';
  pinStatus: string;
  status: 'ACTIVE' | 'INACTIVE';
}

const ADMIN_USERS_DATA: AdminUser[] = [
  {
    id: 'adm-01',
    email: 'stefenlaksana.sl@gmail.com',
    fullName: 'Stefen Laksana (Superadmin)',
    role: 'ROLE_SUPERADMIN',
    scope: 'Seluruh Ekosistem Platform & Database Cloud',
    status: 'ACTIVE',
    createdAt: '2026-08-14',
  },
  {
    id: 'adm-02',
    email: 'ops@newhopepos.id',
    fullName: 'Platform Operations Root',
    role: 'ROLE_SUPERADMIN',
    scope: 'Infrastruktur Server & Gateway API',
    status: 'ACTIVE',
    createdAt: '2026-08-01',
  },
  {
    id: 'adm-03',
    email: 'growth@newhopepos.id',
    fullName: 'Growth & Business Intelligence',
    role: 'ROLE_INTERNAL_GROWTH',
    scope: 'Analisis Metrik Agregat 5 Sektor Usaha',
    status: 'ACTIVE',
    createdAt: '2026-08-05',
  },
  {
    id: 'adm-04',
    email: 'support@newhopepos.id',
    fullName: 'Customer Support Lead',
    role: 'ROLE_INTERNAL_SUPPORT',
    scope: 'Bantuan Merchant & Investigasi Sesi Kasir',
    status: 'ACTIVE',
    createdAt: '2026-08-08',
  },
];

const CLIENT_USERS_DATA: ClientUser[] = [
  {
    id: 'usr-c01',
    storeName: 'Kopi Senja Kemang',
    businessSector: 'FNB',
    name: 'Budi Santoso',
    username: 'budi.admin',
    role: 'ADMIN',
    pinStatus: 'PIN Aktif (1234)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c02',
    storeName: 'Kopi Senja Kemang',
    businessSector: 'FNB',
    name: 'Barista Andi',
    username: 'andi.barista',
    role: 'MANAGER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c03',
    storeName: 'Kopi Senja Kemang',
    businessSector: 'FNB',
    name: 'Kasir Mega',
    username: 'mega.kasir',
    role: 'CASHIER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c04',
    storeName: 'Senja Laundry Kemang',
    businessSector: 'LAUNDRY',
    name: 'Petugas Wati',
    username: 'wati.laundry',
    role: 'MANAGER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c05',
    storeName: 'Senja Laundry Kemang',
    businessSector: 'LAUNDRY',
    name: 'Kasir Lina',
    username: 'lina.kasir',
    role: 'CASHIER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c06',
    storeName: 'Toko Berkah Siti',
    businessSector: 'RETAIL',
    name: 'Siti Aminah',
    username: 'siti.owner',
    role: 'ADMIN',
    pinStatus: 'PIN Aktif (5555)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c07',
    storeName: 'Toko Berkah Siti',
    businessSector: 'RETAIL',
    name: 'Kasir Dewi',
    username: 'dewi.kasir',
    role: 'MANAGER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c08',
    storeName: 'Toko Berkah Siti',
    businessSector: 'RETAIL',
    name: 'Gudang Fajar',
    username: 'fajar.stock',
    role: 'CASHIER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c09',
    storeName: 'Agus Auto Wash',
    businessSector: 'CARWASH',
    name: 'Teknisi Rudi',
    username: 'rudi.wash',
    role: 'MANAGER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c10',
    storeName: 'Agus Auto Wash',
    businessSector: 'CARWASH',
    name: 'Kasir Nina',
    username: 'nina.kasir',
    role: 'CASHIER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c11',
    storeName: 'Rina Beauty Lounge',
    businessSector: 'BARBERSHOP',
    name: 'Kapster Yoga',
    username: 'yoga.barber',
    role: 'MANAGER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
  {
    id: 'usr-c12',
    storeName: 'Rina Beauty Lounge',
    businessSector: 'BARBERSHOP',
    name: 'Kasir Tari',
    username: 'tari.kasir',
    role: 'CASHIER',
    pinStatus: 'PIN Aktif (0000)',
    status: 'ACTIVE',
  },
];

export default function UserManagement() {
  const [activeTab, setActiveTab] = useState<'ADMIN_USERS' | 'CLIENT_USERS'>('ADMIN_USERS');
  const [sectorFilter, setSectorFilter] = useState<string>('ALL');

  const filteredClientUsers =
    sectorFilter === 'ALL'
      ? CLIENT_USERS_DATA
      : CLIENT_USERS_DATA.filter((u) => u.businessSector === sectorFilter);

  return (
    <div className="space-y-6">
      {/* Title & Subtitle */}
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-black text-slate-900 dark:text-white">
            Pemisahan Data Pengguna (Admin vs Client)
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Struktur terisolasi antara User Platform Superadmin (Back-Office) dan User Toko/Merchant (Client POS)
          </p>
        </div>

        <div className="flex gap-2">
          <button
            onClick={() => setActiveTab('ADMIN_USERS')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${
              activeTab === 'ADMIN_USERS'
                ? 'bg-amber-500 text-slate-950 shadow-md'
                : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
            }`}
          >
            <ShieldCheck className="w-4 h-4" />
            <span>User Admin Platform ({ADMIN_USERS_DATA.length})</span>
          </button>

          <button
            onClick={() => setActiveTab('CLIENT_USERS')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${
              activeTab === 'CLIENT_USERS'
                ? 'bg-amber-500 text-slate-950 shadow-md'
                : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
            }`}
          >
            <Users className="w-4 h-4" />
            <span>User Client Merchant ({CLIENT_USERS_DATA.length})</span>
          </button>
        </div>
      </div>

      {/* Info Card Explaining the Isolation Architecture */}
      <div className="p-5 rounded-3xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row gap-6 items-start justify-between">
        <div className="space-y-2 max-w-3xl">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-extrabold border border-emerald-500/30 uppercase tracking-wider">
              Tenant & Schema Isolation Active
            </span>
            <span className="text-xs text-slate-400">Database: Supabase PostgreSQL</span>
          </div>
          <h3 className="text-base font-black text-white">
            {activeTab === 'ADMIN_USERS'
              ? 'Tabel: `internal.internal_users` — Hak Akses Back-Office Platform'
              : 'Tabel: `pos.users` — Hak Akses Terisolasi Per Merchant / Toko'}
          </h3>
          <p className="text-xs text-slate-300 leading-relaxed font-medium">
            {activeTab === 'ADMIN_USERS'
              ? 'Pengguna Admin Platform berwenang melihat metrik agregat, audit trail, performa merchant, dan manajemen lisensi. Data kredensial tidak pernah bercampur dengan database kasir merchant.'
              : 'Pengguna Client Toko terikat pada `tenant_id` masing-masing. Seorang kasir di Kopi Senja tidak dapat mengakses atau melihat data transaksi Toko Berkah atau merchant lainnya.'}
          </p>
        </div>

        <div className="p-3 bg-slate-950 rounded-2xl border border-slate-800 text-xs space-y-1.5 shrink-0 w-full md:w-64">
          <div className="flex items-center justify-between text-slate-400">
            <span>Admin Platform:</span>
            <span className="font-mono text-amber-400 font-bold">{ADMIN_USERS_DATA.length} Akun</span>
          </div>
          <div className="flex items-center justify-between text-slate-400">
            <span>Merchant Staf:</span>
            <span className="font-mono text-emerald-400 font-bold">{CLIENT_USERS_DATA.length} Staf</span>
          </div>
          <div className="flex items-center justify-between text-slate-400">
            <span>Superadmin:</span>
            <span className="font-mono text-white font-bold">Stefen Laksana</span>
          </div>
        </div>
      </div>

      {/* VIEW 1: USER ADMIN PLATFORM (internal.internal_users) */}
      {activeTab === 'ADMIN_USERS' && (
        <Card>
          <div className="p-5 border-b border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-amber-500/20 text-amber-400 rounded-xl">
                <ShieldCheck className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-black text-sm text-white">Daftar Administrator Platform (Internal)</h4>
                <p className="text-xs text-slate-400">Pengguna dengan hak akses khusus ke konsol Back-Office & Monitoring</p>
              </div>
            </div>
            <span className="px-3 py-1 bg-amber-500/10 text-amber-400 text-xs font-bold rounded-xl border border-amber-500/30">
              internal.internal_users
            </span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950/80 text-[11px] text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800">
                <tr>
                  <th className="p-4">Nama Lengkap & Email</th>
                  <th className="p-4">Role Platform</th>
                  <th className="p-4">Lingkup Wewenang (Scope)</th>
                  <th className="p-4">Status</th>
                  <th className="p-4">Terdaftar</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/80">
                {ADMIN_USERS_DATA.map((adm) => (
                  <tr key={adm.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-4">
                      <div className="font-extrabold text-white text-sm">{adm.fullName}</div>
                      <div className="font-mono text-xs text-amber-400">{adm.email}</div>
                    </td>
                    <td className="p-4">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-extrabold border ${
                          adm.role === 'ROLE_SUPERADMIN'
                            ? 'bg-amber-500/20 text-amber-300 border-amber-500/40'
                            : adm.role === 'ROLE_INTERNAL_GROWTH'
                            ? 'bg-blue-500/20 text-blue-300 border-blue-500/40'
                            : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                        }`}
                      >
                        <Shield className="w-3.5 h-3.5" />
                        <span>{adm.role}</span>
                      </span>
                    </td>
                    <td className="p-4 text-slate-300 font-medium">
                      {adm.scope}
                    </td>
                    <td className="p-4">
                      <span className="inline-flex items-center gap-1 text-emerald-400 font-bold">
                        <CheckCircle2 className="w-4 h-4" />
                        <span>{adm.status}</span>
                      </span>
                    </td>
                    <td className="p-4 font-mono text-slate-400">
                      {adm.createdAt}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </Card>
      )}

      {/* VIEW 2: USER CLIENT MERCHANT (pos.users) */}
      {activeTab === 'CLIENT_USERS' && (
        <Card>
          <div className="p-5 border-b border-slate-800 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-blue-500/20 text-blue-400 rounded-xl">
                <Store className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-black text-sm text-white">Daftar Pengguna Client Merchant (pos.users)</h4>
                <p className="text-xs text-slate-400">Owner, Supervisor/Manager, dan Kasir per Tenant Toko</p>
              </div>
            </div>

            {/* Sector Filter */}
            <div className="flex items-center gap-2">
              <Filter className="w-4 h-4 text-slate-400" />
              <select
                value={sectorFilter}
                onChange={(e) => setSectorFilter(e.target.value)}
                className="bg-slate-950 border border-slate-800 text-xs text-slate-200 px-3 py-1.5 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-500"
              >
                <option value="ALL">Semua Sektor ({CLIENT_USERS_DATA.length})</option>
                <option value="FNB">Kafe & Resto (F&B)</option>
                <option value="LAUNDRY">Laundry</option>
                <option value="RETAIL">Ritel & Minimarket</option>
                <option value="CARWASH">Cuci Kendaraan</option>
                <option value="BARBERSHOP">Barbershop</option>
              </select>
            </div>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950/80 text-[11px] text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800">
                <tr>
                  <th className="p-4">Nama Staf & Username</th>
                  <th className="p-4">Toko / Merchant</th>
                  <th className="p-4">Sektor Bisnis</th>
                  <th className="p-4">Role Toko</th>
                  <th className="p-4">Status PIN Kasir</th>
                  <th className="p-4">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/80">
                {filteredClientUsers.map((cli) => (
                  <tr key={cli.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-4">
                      <div className="font-extrabold text-white text-sm">{cli.name}</div>
                      <div className="font-mono text-xs text-slate-400">@{cli.username}</div>
                    </td>
                    <td className="p-4 font-bold text-slate-200">
                      <div className="flex items-center gap-1.5">
                        <Building2 className="w-3.5 h-3.5 text-amber-400" />
                        <span>{cli.storeName}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <span className="px-2 py-0.5 rounded-lg text-[10px] font-extrabold bg-slate-800 text-slate-300 border border-slate-700">
                        {SECTOR_LABEL[cli.businessSector]}
                      </span>
                    </td>
                    <td className="p-4">
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-extrabold border ${
                          cli.role === 'ADMIN'
                            ? 'bg-purple-500/20 text-purple-300 border-purple-500/40'
                            : cli.role === 'MANAGER'
                            ? 'bg-blue-500/20 text-blue-300 border-blue-500/40'
                            : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                        }`}
                      >
                        {cli.role === 'ADMIN' && <UserCog className="w-3 h-3" />}
                        {cli.role === 'MANAGER' && <Briefcase className="w-3 h-3" />}
                        {cli.role === 'CASHIER' && <Store className="w-3 h-3" />}
                        <span>{cli.role}</span>
                      </span>
                    </td>
                    <td className="p-4">
                      <span className="inline-flex items-center gap-1 text-slate-300 font-mono text-[11px]">
                        <KeyRound className="w-3.5 h-3.5 text-amber-400" />
                        <span>{cli.pinStatus}</span>
                      </span>
                    </td>
                    <td className="p-4">
                      <span className="inline-flex items-center gap-1 text-emerald-400 font-bold">
                        <CheckCircle2 className="w-4 h-4" />
                        <span>{cli.status}</span>
                      </span>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </Card>
      )}
    </div>
  );
}
