...
export default function Audit(){
 ...
 const rows=(data?.rows || []).filter((r:any)=>[r.internal_email,r.internal_name,r.action,r.resource,r.justification].join(' ').toLowerCase().includes(search.toLowerCase()));
 ...
 {rows.map((r:any)=><tr key={r.id}><Td>{waktu(r.accessed_at)}</Td><Td>{r.internal_email}<br/><small>{r.internal_role}</small></Td><Td>{r.action}<br/><small>{r.resource}</small></Td><Td>{r.justification || '—'}</Td></tr>)}
