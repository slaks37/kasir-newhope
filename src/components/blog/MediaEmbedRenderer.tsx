...
export const MediaEmbedRenderer: React.FC<MediaEmbedRendererProps> = ({ embed }) => {
    ...
    if (embedUrl) {
      return (
        <figure className="my-6 space-y-2.5">
          ...
          {caption && (
            <figcaption className="text-center text-xs text-slate-500 font-medium flex items-center justify-center space-x-1.5">
  ...
  if (type === 'tiktok') {
    ...
    return (
      <figure className="my-6 space-y-2.5 max-w-sm mx-auto">
        <div className="relative w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 shadow-md min-h-[480px]">
          {embedUrl ? (
            <iframe
              src={embedUrl}
            ...
            <div className="p-6 text-center flex flex-col items-center justify-center h-64 space-y-3">
              <Music2 className="w-10 h-10 text-cyan-600 animate-pulse" />
              <p className="text-xs text-slate-800 font-bold">Video TikTok Tersemat</p>
              <a
                href={url}
                target="_blank"
                rel="noreferrer"
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-900 border border-slate-200 rounded-xl text-xs font-bold flex items-center space-x-1.5 shadow-2xs"
              >
        ...
        {caption && (
          <figcaption className="text-center text-xs text-slate-500 font-medium flex items-center justify-center space-x-1.5">
            <Music2 className="w-3.5 h-3.5 text-cyan-600 shrink-0" />
            <span>{caption}</span>
          </figcaption>
        )}
      </figure>
  ...
  if (type === 'instagram') {
    ...
    return (
      <figure className="my-6 space-y-2.5 max-w-md mx-auto">
        <div className="relative w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 shadow-md min-h-[420px]">
          {embedUrl ? (
            <iframe
              src={embedUrl}
            ...
            <div className="p-6 text-center flex flex-col items-center justify-center h-64 space-y-3">
              ...
              <p className="text-xs text-slate-800 font-bold">Postingan / Reel Instagram</p>
              <a
                href={url}
                target="_blank"
                rel="noreferrer"
                className="px-4 py-2 bg-gradient-to-r from-purple-600 to-rose-600 text-white rounded-xl text-xs font-bold flex items-center space-x-1.5 shadow-xs"
              >
        ...
        {caption && (
          <figcaption className="text-center text-xs text-slate-500 font-medium flex items-center justify-center space-x-1.5">
            <Share2 className="w-3.5 h-3.5 text-rose-500 shrink-0" />
            <span>{caption}</span>
          </figcaption>
        )}
      </figure>
  ...
  return (
    <figure className="my-6 p-4 rounded-2xl bg-white border border-slate-200 shadow-sm flex items-center justify-between gap-4">
      <div className="flex items-center space-x-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-xs font-bold text-slate-900 truncate">{caption || 'Konten Media Tersemat'}</p>
          <p className="text-[11px] text-slate-500 font-mono truncate">{url}</p>
        </div>
      </div>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-amber-800 border border-slate-200 rounded-xl text-xs font-bold shrink-0 flex items-center space-x-1.5 shadow-2xs"
      >
