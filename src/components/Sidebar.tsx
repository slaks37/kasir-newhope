  ...
  return (
    <aside className="nh-sidebar">
      <div>
        ...
        <nav className="nh-sidebar-nav" aria-label="Navigasi toko">
          {navItems.map(item => {
            ...
            return <button key={item.id} aria-current={activeTab === item.id ? 'page' : undefined}
              className={`nh-sidebar-item ${!isAllowed ? 'is-locked' : ''}`}
              title={!isAllowed ? `Akses ${item.label} memerlukan izin Manager / Admin` : undefined}
              onClick={() => item.id === 'ai' ? onOpenAiCopilot() : setActiveTab(item.id)}>
              <Icon /><span>{item.label}</span>
              {!isAllowed && <Lock size={13} className="ml-auto" />}
              {isAllowed && item.badge ? <span className="nh-sidebar-count">{item.badge}</span> : null}
            </button>;
          })}
        </nav>
      </div>
      <div className="nh-sidebar-foot">
        ...
        <button onClick={onOpenClockIn} className="nh-sidebar-item"><UserCheck /><span>Absensi staf<small>{staffMembers.filter(s => getActiveAttendance(s.id)).length} staf sedang bertugas</small></span></button>
        <button onClick={onOpenEndShift} className="nh-sidebar-item">
          <Clock /><span>{shift.status === 'OPEN' ? 'Kelola shift' : 'Mulai shift'}<small>{shift.status === 'OPEN' ? shift.cashierName || currentUser?.name || 'Kasir' : 'Belum ada shift aktif'}</small></span>
          <span className={`nh-status-dot ${shift.status === 'OPEN' ? 'is-open' : ''}`} />
        </button>
        {onGoToHome && <><button onClick={onGoToHome} className="nh-sidebar-item"><Home /><span>Halaman utama</span></button><a href="#blog" className="nh-sidebar-item"><BookOpen /><span>Blog Harapan Baru</span></a></>}
      </div>
    </aside>
  );
};
