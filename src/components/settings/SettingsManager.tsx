            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-lg text-slate-900 flex items-center space-x-2">
                  ...
                  <span>Pengaturan Cetak Struk Kasir &amp; Printer Termal</span>
                </h3>
                <span className="px-2.5 py-1 bg-amber-50 text-amber-800 text-[10px] font-bold rounded-lg border border-amber-200 flex items-center space-x-1">
                  <Printer className="w-3 h-3 text-amber-600" />
                  <span>Format Nota Terkunci</span>
                </span>
              </div>

              <p className="text-xs text-slate-500">
                Atur ukuran standar kertas nota termal agar hasil cetak selalu pas, proporsional, dan konsisten (tidak berubah menjadi ukuran penuh A4).
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
                  <label className="text-xs text-slate-800 font-bold block">
                    Ukuran Kertas Nota / Struk
                  </label>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() => setFormSettings((p) => ({ ...p, receiptPaperSize: '80mm' }))}
                      className={`p-3 rounded-xl border text-left transition-all ${
                        (formSettings.receiptPaperSize || '80mm') === '80mm'
                          ? 'border-amber-500 bg-amber-50 text-slate-900 ring-2 ring-amber-500/20'
                          : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                      }`}
                    >
                      <div className="font-extrabold text-xs">80mm (Standar)</div>
                      <div className="text-[10px] text-slate-500">Kafe, Resto, Ritel, Minimarket</div>
                    </button>

                    <button
                      type="button"
                      onClick={() => setFormSettings((p) => ({ ...p, receiptPaperSize: '58mm' }))}
                      className={`p-3 rounded-xl border text-left transition-all ${
                        formSettings.receiptPaperSize === '58mm'
                          ? 'border-amber-500 bg-amber-50 text-slate-900 ring-2 ring-amber-500/20'
                          : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                      }`}
                    >
                      <div className="font-extrabold text-xs">58mm (Mini)</div>
                      <div className="text-[10px] text-slate-500">Printer Bluetooth / Portabel</div>
                    </button>
                  </div>
                  <p className="text-[10px] text-slate-500">
                    💡 Lebar cetakan nota otomatis dikunci pada format termal fisik agar tidak melar menjadi A4.
                  </p>
                </div>
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 flex flex-col justify-between">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-bold text-xs text-slate-900 block">Cetak Struk Otomatis</span>
                      <span className="text-[11px] text-slate-500">
                        Otomatis memicu dialog print saat pembayaran selesai
                      </span>
                    </div>
                    <input
                      type="checkbox"
                      checked={formSettings.autoPrintReceipt}
                      onChange={(e) => setFormSettings((p) => ({ ...p, autoPrintReceipt: e.target.checked }))}
                      className="w-4 h-4 accent-amber-500 rounded cursor-pointer"
                    />
                  </div>
                ...
                {/* Receipt Header Text */}
                <div className="space-y-1">
                  <label className="text-xs text-slate-700 font-semibold">Teks Header Struk (Salam Pembuka)</label>
                  <textarea
                    rows={2}
                    value={formSettings.receiptHeader || ''}
                    onChange={(e) => setFormSettings((p) => ({ ...p, receiptHeader: e.target.value }))}
                    placeholder="Contoh: Selamat Datang! Terima Kasih Atas Kunjungan Anda"
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>
                ...
                {/* Receipt Footer Text */}
                <div className="space-y-1">
                  <label className="text-xs text-slate-700 font-semibold">Teks Footer Struk (Catatan Penutup)</label>
                  <textarea
                    rows={2}
                    value={formSettings.receiptFooter || ''}
                    onChange={(e) => setFormSettings((p) => ({ ...p, receiptFooter: e.target.value }))}
