...
/*
 * Katalog cadangan saat endpoint paket belum terjawab.
 *
 * Dulu disalin utuh di sini — salinan keempat dari katalog yang sama. Karena UI
 * inilah yang dilihat merchant saat memilih paket, salinan yang tertinggal satu
 * revisi berarti ia MELIHAT harga lama dan DITAGIH harga baru.
 *
 * Sekarang mengimpor sumber yang sama dengan billing-service. Nilai yang
 * ditampilkan tetap diganti oleh jawaban GET /api/v1/subscription/plans begitu
 * tiba; ini hanya mengisi layar pada milidetik pertama.
 */
import { SAAS_PLANS as DEFAULT_SAAS_PLANS } from '../../data/saasPlans';

export const SubscriptionBillingTab: React.FC = () => {
  const { settings, updateSettings, currentUser } = usePOS();

  const [plans, setPlans] = useState<SaaSPlan[]>(DEFAULT_SAAS_PLANS);
