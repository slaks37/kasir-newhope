...
import { billingQuote, manualTierChange, subscriptionAccess, lifecycleStage } from '../../src/config/subscriptionPolicy';
...
async function main(){
 ...
 for(const role of ['anon','authenticated']) for(const table of ['internal.internal_users','billing.payment_events','internal.support_actions','contract.subscription_operations','contract.admin_activity_log']) {
 ...
 await db.query(`INSERT INTO internal.internal_users(id,email,full_name,role,sso_subject) VALUES($1,'growth@test.invalid','Growth','ROLE_INTERNAL_GROWTH','growth-sub')`,[randomUUID()]);
 ...
 registerAdminRoutes(app,async()=>db,async req=>{
  const token=req.headers.authorization;
  if(token==='Bearer merchant')return{subject:'owner-one'};
  if(token==='Bearer no-mfa')return{subject:'admin-sub',aal:'aal1'};
  if(token==='Bearer test-growth')return{subject:'growth-sub',aal:'aal2',mfaVerifiedAt:Date.now()/1000};
  if(token==='Bearer stale-mfa')return{subject:'admin-sub',aal:'aal2',mfaVerifiedAt:Date.now()/1000-601};
  if(token==='Bearer test-admin' || token==='Bearer test-support')return{subject:token==='Bearer test-admin'?'admin-sub':'support-sub',aal:'aal2',mfaVerifiedAt:Date.now()/1000};
  return null;
 });
 let checkoutCalls=0;
 ...
 try{
  ...
  const enroll=await fetch(url+'/api/admin/me',{headers:{authorization:'Bearer no-mfa'}});assert.equal(enroll.status,200);assert.equal((await enroll.json()).mfaRequired,true);
  const callerId=randomUUID();
  const untrustedId=await fetch(url+'/api/admin/me',{headers:{authorization:'Bearer test-admin','x-request-id':callerId}});
  assert.notEqual(untrustedId.headers.get('x-request-id'),callerId);assert.equal(untrustedId.headers.get('cache-control'),'no-store');
  const savedGatewayToken=process.env.INTERNAL_GATEWAY_TOKEN;
  try {
   process.env.INTERNAL_GATEWAY_TOKEN='test-gateway-secret';
   const trustedId=await fetch(url+'/api/admin/me',{headers:{authorization:'Bearer test-admin','x-request-id':callerId,'x-newhope-gateway-token':'test-gateway-secret'}});
   assert.equal(trustedId.headers.get('x-request-id'),callerId);
  } finally { if(savedGatewayToken===undefined)delete process.env.INTERNAL_GATEWAY_TOKEN;else process.env.INTERNAL_GATEWAY_TOKEN=savedGatewayToken; }
  const blocked=await fetch(url+'/api/admin/subscriptions',{headers:{authorization:'Bearer no-mfa','x-aal':'aal2'}});assert.equal(blocked.status,403);assert.equal((await blocked.json()).error,'MFA_REQUIRED');
  const staleMutation=await fetch(url+'/api/admin/tenants/'+tenant+'/support',{method:'POST',headers:{authorization:'Bearer stale-mfa','content-type':'application/json'},body:JSON.stringify({action:'NOTE',reason:'Should not be recorded as success'})});
  assert.equal(staleMutation.status,403);assert.equal((await staleMutation.json()).error,'REAUTH_REQUIRED');
  ...
  assert.equal((await fetch(detailUrl,{headers:{authorization:'Bearer test-growth'}})).status,403);
  assert.equal((await fetch(detailUrl,{headers:{authorization:'Bearer test-support'}})).status,400);
  const detailResponse=await fetch(detailUrl+'?justification=Investigate%20billing%20ticket',{headers:{authorization:'Bearer test-support'}});
  assert.equal(detailResponse.status,200,await detailResponse.clone().text());
  const detail=await detailResponse.json();assert.equal(detail.tenant.id,tenant);assert.equal(detail.capacity.used,2);assert.equal(detail.capacity.max,5);
  assert.equal(detail.subscription.planId,'plan-pro-monthly');assert.equal(detail.derivedTrial,false);
  assert.ok(detail.invoices.some((i:any)=>i.id===invoice));assert.ok(detail.payments.some((p:any)=>p.invoice_number===number));
  assert.ok(detail.payments.every((p:any)=>!('payload' in p)));
  const isolated=await (await fetch(url+'/api/admin/tenants/'+other+'/subscription-detail',{headers:{authorization:'Bearer test-admin'}})).json();
  ...
  const noteAudit=(await db.query('SELECT * FROM internal.support_actions LIMIT 1')).rows[0];assert.equal(noteAudit.request_id,note.headers.get('x-request-id'));assert.ok(noteAudit.ip_address);assert.ok(noteAudit.user_agent);
  assert.ok((await db.query('SELECT id FROM internal.internal_access_log WHERE request_id=$1',[noteAudit.request_id])).rowCount);
  const invoiceNote=await fetch(url+'/api/admin/tenants/'+tenant+'/support',{method:'POST',headers:{authorization:'Bearer test-admin','content-type':'application/json','user-agent':'Audit regression client','x-request-id':randomUUID()},body:JSON.stringify({action:'PAYMENT_NOTE',invoiceId:invoice,reason:'Verified original invoice note'})});
  assert.equal(invoiceNote.status,200);
  const invoiceAudit=(await db.query("SELECT * FROM internal.support_actions WHERE action='PAYMENT_NOTE'")).rows[0];
  assert.equal(invoiceAudit.before_state.invoice.id,invoice);assert.equal(invoiceAudit.after_state.invoice.reconciliation_note,'Verified original invoice note');assert.equal(invoiceAudit.user_agent,'Audit regression client');
  assert.equal(invoiceAudit.request_id,invoiceNote.headers.get('x-request-id'));
  for(const table of ['internal.internal_access_log','internal.support_actions']) {
   for(const operation of ['UPDATE '+table+' SET request_id=request_id','DELETE FROM '+table,'TRUNCATE '+table]) {
    await assert.rejects(()=>db.exec(operation),/ADMIN_AUDIT_APPEND_ONLY/);
    await assert.rejects(()=>db.tx(async c=>{await c.exec('SET LOCAL ROLE svc_internal');await c.exec(operation);}),/permission denied|ADMIN_AUDIT_APPEND_ONLY/);
   }
  }
  console.log('PASS: MFA gate, stale-MFA mutation rejected, correlation metadata, invoice snapshots, audit update/delete/truncate denied');
  ...
  const supportHeaders={authorization:'Bearer test-support'};
  const justification='&justification=Investigate%20regression%20ticket';
  const realSale=(await db.query("SELECT id FROM pos.transactions WHERE client_txn_id='real-sale-1'")).rows[0].id;
  const txnPath=url+'/api/admin/transactions/'+realSale;
  assert.equal((await fetch(txnPath+'?merchantId='+merchant+justification,{headers:supportHeaders})).status,404);
  assert.equal((await fetch(txnPath+'?merchantId='+product.merchant_id+justification,{headers:supportHeaders})).status,200);
  for(const badScope of ['------------------------------------','merchantId[]='+merchant]) {
   const param=badScope.includes('=')?badScope:'merchantId='+badScope;
   assert.equal((await fetch(url+'/api/admin/transactions?'+param+justification,{headers:supportHeaders})).status,400);
  }
  const breakdown=await (await fetch(url+'/api/admin/activity/breakdown?merchantId='+merchant+justification,{headers:supportHeaders})).json();
  assert.ok(!breakdown.rows.some((r:any)=>r.event_type==='TEST_OPERATION'));
  const ownBreakdown=await (await fetch(url+'/api/admin/activity/breakdown?merchantId='+product.merchant_id+justification,{headers:supportHeaders})).json();
  assert.ok(ownBreakdown.rows.some((r:any)=>r.event_type==='TEST_OPERATION'));
  for(const who of ['test-support','test-growth']) {
   const directory=await (await fetch(url+'/api/admin/merchants',{headers:{authorization:'Bearer '+who}})).json();
   assert.equal(directory.financialDetail,false);
   for(const row of directory.rows) for(const key of ['email','phone','owner_name','gross_revenue','gross_profit','transaction_count','owner_user_ref']) assert.ok(!(key in row),key);
  }
  const paidBefore=await ensureSubscription(db,tenant);
  const override=manualTierChange(paidBefore,{planId:'plan-plus-monthly',billingCycle:'YEARLY',extraOutlets:0},2);
  assert.equal(override.periodEnd,new Date(paidBefore.current_period_end).toISOString());
  assert.equal(override.recurringAmount,Number(paidBefore.recurring_amount));
  assert.equal((await act('GRANT_PLAN',{planId:'plan-plus-monthly',billingCycle:'MONTHLY'},'test-admin',tenant)).status,400);
  assert.equal((await act('GRANT_PLAN',{planId:'plan-plus-monthly',billingCycle:'YEARLY',extraOutlets:0},'test-admin',tenant)).status,200);
  const paidAfter=await ensureSubscription(db,tenant);
  assert.equal(paidAfter.plan_id,'plan-plus-monthly');
  assert.equal(new Date(paidAfter.current_period_end).toISOString(),new Date(paidBefore.current_period_end).toISOString());
  assert.equal(Number(paidAfter.recurring_amount),Number(paidBefore.recurring_amount));
  await assert.rejects(()=>db.tx(c=>assertOutletCapacity(c,tenant)),/OUTLET_LIMIT_REACHED/);
  assert.equal(new Date(paidAfter.current_period_start).toISOString(),new Date(paidBefore.current_period_start).toISOString());
  await db.query('UPDATE internal.internal_users SET is_active=false WHERE id=$1',[admin]);
  assert.equal((await act('GRANT_PLAN',{planId:'plan-pro-monthly'})).status,403);
  await db.query('UPDATE internal.internal_users SET is_active=true WHERE id=$1',[admin]);
  await db.tx(async c=>{await c.exec('SET LOCAL ROLE svc_internal');assert.ok((await c.query('SELECT id FROM internal.internal_users')).rowCount>0);});
  for(const role of ['anon','authenticated','svc_internal','svc_pos','svc_ai','svc_billing']) {
   for(const sql of ["UPDATE internal.internal_users SET role='ROLE_SUPERADMIN'",'DELETE FROM internal.internal_users','TRUNCATE internal.internal_users',
    "INSERT INTO internal.internal_users(email,full_name,role) VALUES('attacker@test.invalid','Attacker','ROLE_SUPERADMIN')"]) {
    await assert.rejects(()=>db.tx(async c=>{await c.exec('SET LOCAL ROLE '+role);await c.exec(sql);}),/permission denied/);
   }
  }
  console.log('PASS: support receipt/breakdown scope, malformed scope rejection, directory privacy, revoked admin denial, paid tier override preserves term and value');
  ...
  process.env.DOKU_CLIENT_ID='test-client';process.env.DOKU_SECRET_KEY='test-secret';process.env.PUBLIC_APP_URL='https://test.invalid';process.env.DOKU_ALLOWED_CHANNELS='VIRTUAL_ACCOUNT_BCA';
